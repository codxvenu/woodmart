...
const Orders = () => {
  ...
  async function Download(id) {
    const address =  JSON.parse(localStorage.getItem("form"))
    if(!address) console.log(address,"erro");
      ...
      <table className='w-full'>
        ...
        <tbody>
          {orders.map((order)=>(
            <tr key={order.id}>
                <td>#{order.id}</td>
                <td><button className='text-white bg-[#f59a57] px-6 py-3 w-fit text-[13px] rounded-3xl font-medium' onClick={()=>Download(order.id)}>Download</button></td>
            </tr>
          ))}
        </tbody>
      </table>
